...
export const wishlistSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentBooks: (state, action) => {
      state.currentBooks = [
        ...new Map(
          action.payload.currentBooks.map((book) => [book.isbn, book])
        ).values(),
      ];
    },
    setDumpedBooks: (state, action) => {
      state.dumpedBooks = [
        ...new Map(
          action.payload.dumpedBooks.map((book) => [book.isbn, book])
        ).values(),
      ];
    },
    setPreviousBooks: (state, action) => {
      state.previousBooks = [
        ...new Map(
          action.payload.previousBooks.map((book) => [book.isbn, book])
        ).values(),
      ];
    },
    ...
    addToWishlist: (state, action) => {
      ...
      if (
        !state.wishlist.find((book) => book.isbn === action.payload.book.isbn)
      )
    ...
    addToPreviousBooks: (state, action) => {
      state.dumpedBooks = state.dumpedBooks.filter(
        (book) => book.isbn !== action.payload.book.isbn
      );
      if (
        !state.previousBooks.find(
          (book) => book.isbn === action.payload.book.isbn
        )
      )
        state.previousBooks = [
          ...state.previousBooks,
          { ...action.payload.book, userRating: 0 },
        ];
    },
    ratePreviousBooks: (state, action) => {
      state.previousBooks = state.previousBooks.map((book) => {
        if (book.isbn === action.payload.book.isbn)
          return { ...book, userRating: action.payload.userRating };
        return book;
      });
    },
