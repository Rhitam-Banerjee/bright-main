...
const SearchAuthors = ({ data }) => {
  ...
  const getBooks = async (id) => {
    try {
      const response = await axios
        .get(
          age === "" || age === undefined
            ? `${urls.getBooksFromAuthor}?author_id=${id}`
            : `${urls.getBooksFromAuthor}?author_id=${id}&age=${age}`
        )
        .then((res) => res.data)
  ...
  return (
    <section className="pl-8 md:px-2 pb-[10px]">
      <h1 className="flex font-bold text-[12px] pb-[10px]">Authors</h1>
      {data?.length > 0 ? (
        <>
          <Swiper
            slidesPerView={"auto"}
        ...
        <span>No Authors of This Name</span>
      )}
      <div
        className={`${
          bookLoaded && chosen ? "mt-0" : "mt-[14px]"
